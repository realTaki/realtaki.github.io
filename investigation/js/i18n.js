/* ================================================================
   i18n — bilingual switch (zh / en)
   - One dictionary per language, flat keys ("dot.path" style).
   - Static strings in index.html are tagged with data-i18n / data-i18n-attr.
   - Dynamic strings from render.js / modal.js read through i18n.t(key, vars).
   - Persistence priority: URL hash > localStorage > <html lang>.
   ================================================================ */
(function () {
  const LANG_KEY = 'agent-lands.lang';
  ...
  const dict = {
    zh: {
      ...
      // 模块卡 (render.js 中的 forEach(Object.entries(moduleMeta)) 部分)
      'mod.aiSec.title':            'AI Security · 专项高亮',
      'mod.aiSec.count':            '{n} 家公司 · 含完整融资数据',
      'mod.badge.new':              'NEW',

      // 模块描述 (moduleMeta.desc)
      ...
      'hero.ins.lede':              '从产品图谱、融资节奏与并购信号中提炼的三条趋势,帮你快速抓住 AI Infra / AI Security 赛道当前最值得关注的拐点。',
      'hero.ins.meta.view':         '视角',
      'hero.ins.meta.viewVal':      'AI Infra × AI Security',
      'hero.ins.meta.window':       '时间窗',
      'hero.ins.meta.source':       '来源',
      'hero.ins.meta.sourceVal':    '同源 items[] 数据派生',

      // 观察卡
      'insight1.title':             '护栏赛道融资节奏 · {first}-{to}',
      'insight1.body':              '{years}。共 {n} 笔已披露融资,合计 ${total}M —— 资本对 LLM 应用层防护的投入仍在持续。',
      'insight1.body.empty':        '暂无已披露的融资记录。',
      'insight2.title':             '大厂开始扫货 · {n} 起收购',
      'insight2.title.empty':       '尚未出现并购',
      'insight2.body':              '{list} 于 {years} 年先后被收购,合计 ~${total}M 退出,头部整合信号明确。',
      'insight2.body.empty':        '目前数据中还没有已完成的收购记录。',
      'insight3.title':             '{name} 值得关注',
      'insight3.title.empty':       '暂无高亮公司',
      'insight3.body':              '{parts}。',
      'insight3.body.empty':        '请在 meta.js 的 HIGHLIGHT_NAME 里指定一家已存在于 items[] 的公司。',
      'insight.body.location':      '总部 {v}',
      'insight.body.funding':       '{label} 完成 ${amount}M {stage}',
      'insight.body.investors':     '{v} 参投',
      'insight.body.products':      '产品矩阵 {n} 条线',
      'insight.acquired':           '被收购',
      'insight.readMore':           '阅读长文 →',
      ...
      'modal.article.badge':        '长文 · {eyebrow}',
      'modal.article.related':      '关联公司',
      'modal.article.notListed':    '{name} — 未收录',
  ...
  const i18n = {
    ...
    /** Look up a key in the active language. Optional vars fill {placeholders}. */
    t(key, varsOrFallback, maybeFallback) {
      // t(key)                 → string (or key if missing)
      // t(key, varsObj)        → string with placeholders filled
      // t(key, fallbackString) → string (fallback used if key missing)
      // t(key, varsObj, fallbackString)
    ...
    /** Resolve a description for an item: prefers en + desc_en in EN mode, else desc. */
    descFor(item, vars) {
      if (!item) return '';
    ...
    /** Switch language, persist, update <html lang>, reapply static nodes. */
    setLang(lang, opts) {
      ...
      // Mirror to URL hash, preserving page route (e.g. #investigation?lang=en)
      try {
        const raw = (location.hash || '').replace(/^#/, '');
        const [path, query] = raw.split('?');
        const params = new URLSearchParams(query || '');
        params.set('lang', next);
        const newHash = '#' + (path || 'investigation') + '?' + params.toString();
      ...
      // Update document.title from i18n (key falls back to current title if missing)
      try {
        const t = dict[next]['site.title'];
        if (t) document.title = t;
      } catch (e) {}

      // Update CSS custom property for pseudo-element translation (insight::after etc.)
      try {
    ...
    /** Walk [data-i18n] / [data-i18n-attr] and rewrite. */
    applyStatic() {
      document.querySelectorAll('[data-i18n]').forEach(el => {
        const key = el.dataset.i18n;
        // For elements whose current text is purely the key marker (placeholder),
        // it's safe to use innerHTML so markup like <span class="accent"> survives.
        // For real DOM nodes we use textContent unless the i18n string contains markup.
      ...
      document.querySelectorAll('[data-i18n-attr]').forEach(el => {
        const spec = el.dataset.i18nAttr; // e.g. "placeholder:searchPh" or "title:filter.starTitle"
    ...
    /** Resolve initial language with priority: hash > localStorage > <html lang>. */
    resolveInitialLang() {
      try {
        const raw = (location.hash || '').replace(/^#/, '');
    ...
    bootstrap() {
      ...
      // Apply without firing event (listeners may not exist yet).
      this.setLang(_lang, { silent: true });
    },

    /** Wire up the .lang-toggle button(s). */
    bindToggle() {
      document.querySelectorAll('.lang-toggle').forEach(btn => {
        btn.addEventListener('click', (e) => {
          // Allow clicking either side to pick that language directly.
  ...
  /* ----- AUTO-INIT ----- */
  // i18n.js is loaded synchronously at the start of the script block,
  // so by the time this runs, the lang-toggle button has been parsed but
  // data.js / render.js haven't yet. We can safely:
  //   1. resolve the initial lang so renderStats() picks the right strings
  //   2. wire up the toggle click handler (button is in <header>, parsed)
  //   3. apply static i18n — defer until DOMContentLoaded if DOM still parsing
  (function autoInit() {
    _lang = i18n.resolveInitialLang();
    i18n.bindToggle();
    const applyNow = () => i18n.applyStatic();
